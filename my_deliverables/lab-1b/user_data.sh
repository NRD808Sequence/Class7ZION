...
def get_conn():
    ...
    db = c.get("dbname", "labmysql")  # we'll create this if it doesn't exist
...
def init_db():
    ...
    cur = conn.cursor()
    cur.execute("CREATE DATABASE IF NOT EXISTS labmysql;")
    cur.execute("USE labmysql;")
    cur.execute("""
    ...
    cur.close()
    conn.close()
    return "Initialized labmysql + notes table."
